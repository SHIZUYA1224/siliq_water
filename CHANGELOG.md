...
## [2.4.0-wip] - 2026-07-05 (実験ブランチ: claude/water-extra-materials-wip)

> このバージョンは Codex 側の本流開発と分離した実験ブランチです。
> 良い部分だけを後から本流へマージする前提で、単体で完結する形にしてあります。

### 追加: 素材系の表現を大幅拡張 (透明水〜ガラス〜金属)
- 新しい波レイヤータイプ 3 種を追加:
  - `FlutedRibs` (縦リブ / フルートガラス、余弦断面で滑らか)
  - `MetaBlobs` (メタボール / 液体金属・水滴。5x5 近傍合算でタイル境界の継ぎ目なし)
  - `Kaleidoscope` (放射状カットガラス。※中心対称のため非タイリング、装飾パネル向け)
- プリセットを 6 種追加: 透明な浅瀬 / リブガラス / 液体金属 / マットガラス /
  結露ガラス / 万華鏡
- PrebakedPack に上記 6 種の焼き込みテクスチャ + マテリアル + プレビューを追加
  (1024px, iOS/Android ASTC 6x6 込み。万華鏡のみ Clamp ラップ)
- 右クリック「水マテリアルを適用」に素材系 6 種を追加
- パーリン/リッジの「引き伸ばし」がスクロール方向 (縦横の近い方) に沿うよう改善
  (結露の縦の垂れ筋などに対応)
