...
### 修正: 屋外への反射追従と速度スライダーの実効範囲

- Built-in / Quest水面がUnityのReflection Probe / SkyboxをHDR cubemapとして直接サンプリングするよう変更。固定の空色だけで描かず、夕景・森・室内など周囲の色温度と明暗へ追従
- Box Projectionへ対応し、室内Reflection Probeの位置ずれを軽減。手動Reflection Cubeを指定した場合は従来どおり最優先
- `WaterSurfaceAnimator`の速度換算を再設計し、`Speed 0.3`を目視できる中速、`0.6`を明確な高速として反映
- 完成Material 9種のscrollを再調整し、Animatorを付けずMaterialをドラッグするだけでも停止して見えない速度へ改善
- Built-in / URP両シェーダーへ`水面全体の速度倍率`を追加。Material単体でもnormal、macro、高さ、水底光をまとめて速度調整可能
- Animator使用時はMaterial側の速度倍率を自動統合し、二重に速度が掛からないよう修正

## [2.4.3] - 2026-07-27

### 修正: 白い霞・微細波ノイズ・停止して見える高さを再調整

- 完成Material 9種の3枚目の微細normalを初期OFFへ戻し、2層の手調整済み波形を標準化。近距離微細波はPC接写向けの任意機能として残し、Quest / iOSの余分なtexture sampleも削減
- 透過光をベース色へ加算する処理を廃止し、水色へエネルギー保存型で混合。透明度を上げた時の白い霞と色抜けを抑制
- 反射をF0=2%基準のSchlick型フレネルへ変更し、強い反射でも正面全体が白くなる状態を抑制
- 帯状反射を白色加算ではなく反射色への混合へ変更し、normalとmacro波で帯を曲げて直線的な人工模様を軽減
- 高さ速度を秒基準の0.025-0.08へ移行。旧2.4.2以前の極小値もshader内で自動変換し、一周期に何時間もかかる停止状態を解消
- Directional / Point Lightの方向計算を修正
- RendererのMaterialを差し替えた時、`WaterSurfaceAnimator`が新しい色・透明度・反射・normal強度・tiling・2層の流向を自動同期し、前のMaterial設定で上書きしないよう修正
- 回帰テストを追加し、微細波の初期ON、透過光の単純加算、反射帯の白色加算、極小高さ速度への再発を禁止

## [2.4.2] - 2026-07-27

### 修正: 波がザラついて見える問題と、反射色が指定値から離れる問題

2.4.1 で色と明るさは戻したが、「波が騒がしい・ザラつく」がまだ残っていた。原因は 2 点。

- **レイヤー 2 の基準角を 0.61rad (約 35 度) 回していた**のを 0 へ戻した。
  細い光の揺らぎ (`_GlimmerIntensity` / `_GlimmerSharpness`) は
  `n1` と `n2` の干渉から作られているため、レイヤー 2 を回すと干渉パターンが丸ごと変わり、
  `_GlimmerSharpness` 22 のような鋭いきらめきが再配置されてザラつきに化けていた。
  相関崩しは、距離で消える微細レイヤー側だけで行うよう変更
- **微細波の距離フェードを 2 乗から 3 乗**へ変更。画面全体に薄く乗っていた微細波を
  本当に足元だけへ寄せ、「情報量」ではなく単なるザラつきになる状態を解消
- 環境プローブから借りる色味を `_SkyGradient × 0.75` から `× 0.35` へ縮小。
  指定した `_HorizonColor` から反射色が離れないよう、馴染ませる程度にとどめた
- レイヤー 2 の基準角に定数を足さないこと、微細波を 3 乗で落とすことを検証する EditMode test を追加

## [2.4.1] - 2026-07-27

### 修正: 2.4.0 で完成 Material の見た目が劣化していた問題

2.4.0 は「見た目を変える新機能」を既定で有効にしてしまい、手調整済み Material 9 種の
色・明るさ・凹凸量が意図せず変わっていた。新機能は残したまま、既定値では
**2.3.89 の見た目を保つ**よう作り直した。

- **反射が暗く青くなっていた**: 空の階調が `_ZenithColor` をそのまま乗算していたため、真上を向くほど反射が暗くなっていた。真上の色を輝度 1 へ正規化し、階調は「色味の配分」だけを変えて明るさは `_HorizonColor` のまま保つよう修正
- **白い霞がかかっていた**: `_SunSheen` の広いローブが `_SpecIntensity` (完成 Material では最大 2) と掛け算されて水面全体に乗っていた。既定値を 0 (opt-in) にし、`_SpecIntensity` との連動を切ってフレネルで押さえるよう修正。完成 Material 9 種も 0 に設定
- **凹凸が強くなっていた**: 3 枚目のノーマルレイヤーを単純加算していたため、`_NormalStrength` を変えていないのに起伏の総量が増えていた。レイヤー数で正規化し、2 レイヤー時代と同じ強さになるよう修正
- **近〜中距離まで平板になっていた**: 遠景対策の判定が 1 テクセル/ピクセルから立ち上がり、法線を最大 0.30 倍まで潰していた。判定を 2 テクセル/ピクセル以降からに変え、係数も 0.45→0.30 へ緩和。法線の減衰は 0.78 倍までにとどめ、チラつき対策の主役はハイライトのローブ側へ寄せた
- **ハイライトと反射帯が遠景で消えすぎていた**: 強度の減衰を緩和 (ハイライト 0.35→0.72 倍、反射帯 0→0.65 倍、きらめき 0→0.45 倍)
- macro noise を 2.3.89 と同じ 3 波へ戻し、大きなムラの模様を変えないよう修正
- 完成 Material 9 種の `_DetailStrength` を 0.15-0.30 へ、`_SkyGradient` を 0.18-0.45 へ引き下げ。透明感優先の Hero / Crystal Lagoon はとくに弱く設定
- 上記が再発しないよう、エネルギー保存・輝度保存・既定値 OFF を検証する EditMode test を追加
...
#### 両シェーダー共通
- ノーマルを 2 レイヤーから **3 レイヤー**(大・中・微細)へ拡張し、レイヤーごとに回転と非整数比のタイリングを与えて同じテクスチャでも格子状の相関が出ないように変更
- `_DetailStrength` / `_DetailTiling` / `_DetailDistance` を追加。近距離だけに微細波を乗せ、距離で消すためモアレにならず足元の情報量だけが増える
- `_SpecularAA` を追加。1 ピクセルが跨ぐテクセル数 (`fwidth` × texel size) からサンプリング不足を測り、足りない距離では法線を寝かせてハイライトのローブを広げる。遠景で白い点が明滅する現象を解消
- 微細波の分岐条件をマテリアル定数のみにし、ピクセルごとに変わる流れの中でテクスチャの暗黙微分が未定義にならないよう修正
- macro noise を 3 波から 4 波の非整数比へ変更し、大きなうねりの繰り返しを見えにくく変更

#### `Siliq/Water Mobile (Quest)` (完成 Material 9 種が使用)
- `_ZenithColor` / `_SkyGradient` を追加。真上ほど濃く水平線ほど明るい階調のある空を反射し、さらに反射方向の環境プローブ (SH) の色味を借りてシーンごとの空気感に馴染ませる (環境光が暗いシーンでは指定色をそのまま使用)
- `_SunSheen` を追加。鋭い点に加えて広いローブを足し、水面に伸びる光の道を表現
- フレネルに水の垂直入射反射率 2% の下限を追加し、真上から見た時に反射が完全に消えないよう修正
- VR (single-pass instanced) で両目の中点ではなく実際の目の位置を使うよう修正。左右でハイライトがずれる問題を解消
- 水底 caustics のサンプリングをマテリアル定数で分岐させ、初期状態 (caustics OFF) で 4 回のテクスチャ取得を丸ごと省略
- 完成 Material 9 種すべてに用途別のちらつき防止・微細波・空の階調・真上の反射色を設定

#### `Siliq/Water URP` (任意 Sample)
- unlit から**ライティング対応**へ変更。メインライトの影、追加ライト、環境光 (SH) を受ける
- **本物のスクリーンスペース屈折**を追加 (`_UseRefraction`)。`_CameraOpaqueTexture` から背景を取得し水面法線で歪ませる。水面より手前の物体は深度で棄却するため吸い込みが起きない
- **Beer-Lambert 吸収**による水深カラーへ変更。`_DeepColor` を「`_AbsorptionDepth` だけ進んだ時に残る透過率」として扱い、浅い所は水底が素通り、深い所は自然に濃くなる。深度テクスチャがない場合は視線角から光路長を近似
- **波頭の透過光 (SSS)** を追加 (`_SssStrength` / `_ScatterColor`)
- **大きなうねり (スウェル)** を追加。`_SwellNormalStrength` はテクスチャ非依存なので遠景でも繰り返しが見えず、`_SwellHeight` で頂点変位も可能。水平でない面では自動的に効果を弱める
- スペキュラを GGX へ変更し、反射率と同じフレネルで重み付け。真上から見た時に光源が焼き付かない
- コースティクスを深度から復元した**水底のワールド座標へ投影**するよう変更。視点が動いても模様が水面に貼り付いて滑らない。`_CausticsDepthFade` で深いところほど光が届かなくなる
- 岸辺フォームを 2 方向へ流し、しきい値で溶かして泡の縁を作るよう変更。`_EdgeSoftness` で水際をなじませてポリゴンの切り口を隠す
- `_NormalMap_TexelSize` と `_UseRipples` を `UnityPerMaterial` へ移し、SRP Batcher 互換を回復

#### 生成コア
- ノーマルマップの勾配を Sobel から **Scharr** (3, 10, 3) へ変更。回転対称性が高く、斜め方向の波で軸沿いのギザギザが出にくい
- foam / caustics のぼかしを 3x3 ボックスの反復から**分離型ガウシアン**へ変更し、十字状のにじみを解消。半径を解像度に比例させ、512px でも 2048px でも同じ見た目に統一
- 8bit 書き出しに**順序ディザ** (4x4 Bayer, ±0.5LSB) を追加 (`dither8Bit`)。穏やかな水面のグラデーションに出る縞を均す。0 と 255 は保たれ、法線長の精度を守るためノーマルマップには掛けない
- スーパーサンプリングを最大 **4x** へ拡張。収まらない場合は等倍に落とさず、収まる最大の倍率まで段階的に下げる
...
- 通常の `Tools > Siliq Water > 水面マップスタジオ` を、PC / Quest / iOS と完成済み見た目 Material を選んで選択中の水面へ反映する軽量画面へ変更
- 重い手続き生成スタジオは `Tools > Siliq Water > 上級者向け > 水面マップ生成スタジオ` へ移動し、通常導線から外した
- 水面FXに編集時プレビューAnimatorと見える速度を追加し、雨波紋は横スライドではなく中心から広がる表現へ変更
- 水槽・水中カメラ・水中通路向けに、水中の霞、漂う粒子、斜め光筋、泡柱を複数Planeで配置する水中・水槽 FX 空間を追加
...
- 水しぶき、泡、水面反射、水中粒子、岸泡、雨波紋を、6分割アトラス切り出しではなく用途別に個別生成した高密度マスクへ差し替え
- 黒地余白、柔らかい中間階調、焦点ハイライト、白飛び上限をテストで固定し、白い板状・薄すぎる・ノイズだけの素材へ戻らないようにした
- 情報量が増えた泡 / 岸泡 / 水面反射 material の alpha と mask focus を再調整し、重ねた時に潰れにくくした
...
- `preview_crystal_lagoon.png` を、単色の青い抽象背景ではなく、室内プールの透明水、窓反射、水底光が読めるサムネイルへ差し替え
- テストを追加し、Crystal Lagoon preview が青いだけの汚い画像へ戻らないよう、室内背景、暗い反射、明暗差、白飛び上限を固定
...
### 修正: 水面速度をさらに静かな基準へ調整
- `WaterSurfaceAnimator` の内部 UV 速度倍率をさらに下げ、`Speed 0.3` でも水面が速く流れて見えないようにした
- Siliq 水 shader の初期 scroll / 高さ速度 / 水底光速度、Quick Apply / Studio 生成 / 初心者修復 / ReadyMaterial の速度を低速基準へ統一
- テストと README の速度基準を更新し、再び速い初期値へ戻らないよう固定

## [2.3.72] - 2026-07-09

### 追加: 最高品質 Hero 完成セットを一発配置
- `Tools > Siliq Water > かんたん作成 > 最高品質 Hero 完成セットを配置` と GameObject menu を追加し、水面、明るい床、Hero caustics overlay、確認用ライトをまとめて置けるようにした
- はじめてガイドの最初の導線を「水面だけ作る」から「完成セットを配置」へ寄せ、初心者が透明感と水底光を同時に確認できるようにした
- README / 用途別ガイド / テストを更新し、最高品質確認は Hero 完成セットから始める導線を固定

## [2.3.71] - 2026-07-09

### 改善: 暗所でも水底光が消えすぎないよう調整
- `Siliq/Water Mobile (Quest)` の caustics / 柔らかい水底光膜を、反射やきらめき用の暗所減衰だけに依存しない専用 visibility へ変更
- 暗い室内プールや夜寄りのシーンでも、Crystal Lagoon / Hero の水底光が完全に死なず、透明水の底光として残るようにした
- shader ソーステストを追加し、焦点線 caustics と柔らかい水底光膜が暗所で消えすぎる式へ戻らないよう固定
