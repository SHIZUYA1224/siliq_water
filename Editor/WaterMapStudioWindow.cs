...
{
    /// <summary>
    /// 既存プロジェクトとの互換用に残す手続き生成画面。
    /// 製品の通常メニューには公開せず、完成済み Material を標準導線にする。
    /// </summary>
    public class WaterMapStudioWindow : EditorWindow
    {
        ...
        void CreateMaterial(string path, List<(string path, WaterMapType type)> maps)
        {
            ...
            switch (resolvedShaderIndex)
            {
                ...
                case 4: // Siliq URP
                    ...
                    SetupSiliqCaustics(mat, caustics, 0.34f, 1.8f, 0.000010f, new Color(0.78f, 1f, 1f, 1f));
                    if (height != null && mat.HasProperty("_HeightMap"))
                    {
                        ...
                        if (mat.HasProperty("_DisplacementSpeed")) mat.SetFloat("_DisplacementSpeed", 0.000065f);
                    }
                    else
                    {
        ...
        static void SetupSiliqMobileTransparent(Material mat, float opacity)
        {
            ...
            SetupSiliqCaustics(mat, null, 0.30f, 1.8f, 0.000010f, new Color(0.78f, 1f, 1f, 1f));
            if (mat.HasProperty("_SrcBlend")) mat.SetFloat("_SrcBlend", (float)BlendMode.SrcAlpha);
            if (mat.HasProperty("_DstBlend")) mat.SetFloat("_DstBlend", (float)BlendMode.OneMinusSrcAlpha);
            if (mat.HasProperty("_ZWrite")) mat.SetFloat("_ZWrite", 0f);
        ...
        static void SetupSiliqIndoorBluePool(Material mat, float opacity, bool hasHeightMap)
        {
            ...
            if (mat.HasProperty("_DisplacementSpeed")) mat.SetFloat("_DisplacementSpeed", 0.000045f);
            if (mat.HasProperty("_HeightMapInfluence")) mat.SetFloat("_HeightMapInfluence", hasHeightMap ? 0.25f : 0f);
            SetupSiliqCaustics(mat, null, 0.58f, 1.7f, 0.000007f, new Color(0.82f, 0.98f, 1f, 1f));
        ...
        static void SetupSiliqFlagshipCrystal(Material mat, float opacity, bool hasHeightMap)
        {
            ...
            if (mat.HasProperty("_DisplacementSpeed")) mat.SetFloat("_DisplacementSpeed", 0.000045f);
            if (mat.HasProperty("_HeightMapInfluence")) mat.SetFloat("_HeightMapInfluence", 0f);
            SetupSiliqCaustics(mat, null, 0.64f, 2.1f, 0.000007f, new Color(0.76f, 1f, 0.98f, 1f));
        ...
        static void SetupSiliqCaustics(Material mat, Texture2D caustics, float strength, float scale, float speed, Color tint)
        {
            ...
            if (mat.HasProperty("_CausticsScatterStrength")) mat.SetFloat("_CausticsScatterStrength", Mathf.Lerp(0.24f, 0.58f, Mathf.Clamp01(strength)));
            if (mat.HasProperty("_BottomLightStrength")) mat.SetFloat("_BottomLightStrength", Mathf.Lerp(0.9f, 1.7f, Mathf.Clamp01(strength)));
            if (mat.HasProperty("_BottomGlowStrength")) mat.SetFloat("_BottomGlowStrength", Mathf.Lerp(0.42f, 0.82f, Mathf.Clamp01(strength)));
            if (mat.HasProperty("_DepthTintStrength")) mat.SetFloat("_DepthTintStrength", Mathf.Lerp(0.20f, 0.34f, Mathf.Clamp01(strength)));
