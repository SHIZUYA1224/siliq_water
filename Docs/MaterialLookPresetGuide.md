...
Hero / Crystal Lagoon / 透明プールは、置いた瞬間に水面が滑って見えないよう shader scroll と Animator speed をかなり低速にしている。もっと静止に近づけたい場合は `Speed` を 0.01 以下、または material の `_Scroll1` / `_Scroll2` を 0 にする。水底光まで速く見える場合は `_CausticsSpeed`、高さの動きが速い場合は `_DisplacementSpeed` を下げる。Hero の水底光は直線格子ではなく、有機的に曲がる caustics の光筋として調整する。透明水の平板さが気になる場合は `_RefractionStrength` を 0.25-0.45 の範囲で使い、水底光と反射を薄く歪ませる。
