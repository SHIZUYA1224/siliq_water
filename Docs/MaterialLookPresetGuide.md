...
Unity / VRChat 初心者は、まず `Tools > Siliq Water > かんたん作成 > フラッグシップ水面を作成` を使う。分割済みメッシュ、マテリアル、Animator、最低限のライト/カメラが一括で作られる。既存の水面がピンク、動かない、高さが出ない場合は `選択中の水面を診断して自動修復` を実行する。
