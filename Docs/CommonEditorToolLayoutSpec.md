...
生成系ツールでは、技術的なプリセット一覧の前に「目的から始める」入口を置く。
...
- 用途が明確なツールでは、最初に「綺麗な海」「透明プール」のような目的別ボタンを置き、レシピ、品質、出力対象、マテリアル設定までまとめて推奨値にする
...
- `軽量` / `高品質` / `最高品質` のような品質ショートカットを用意し、解像度やスーパーサンプリングを一括変更できるようにする
