...
- **本物のスクリーンスペース屈折**: 「本物の屈折を使う」を ON にすると `_CameraOpaqueTexture` から背景を取得し、
  ...
  (URP 設定で **Opaque Texture** と **Depth Texture** を ON にしてください)
- **Beer-Lambert 吸収による水深カラー**: `_DeepColor` を「`_AbsorptionDepth` だけ進んだ時に残る透過率」として扱い、
  浅いところは水底が素通り、深いところは自然に濃くなります
- **波頭の透過光 (SSS)**: 太陽を背にした波が `_SssStrength` / `_ScatterColor` で明るく透けます
- **大きなうねり (スウェル)**: `_SwellNormalStrength` はテクスチャに依存しない波形なので、遠景でも繰り返しが見えません。
  `_SwellHeight` を上げるとメッシュの頂点も実際に持ち上がります(分割数の多いメッシュが必要)
- GGX スペキュラ + `_SunSheen` による水面に伸びる光の道 + リフレクションプローブ + フレネル (垂直入射 2%)
- **水底へ投影されるコースティクス**: 深度から水底のワールド座標を復元して投影するため、
  視点が動いても模様が水面に貼り付いて滑りません。`_CausticsDepthFade` で深いところほど光が届かなくなります
- **深度ベースの岸辺エフェクト**(浅瀬の色変化・2 方向へ流れる岸辺フォーム・`_EdgeSoftness` による水際のなじませ)
