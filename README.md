...
ガイド内のボタンから、最高品質 Hero 水面の作成、クリスタルラグーン水面の作成、完成 Prefab の選択、選択中の水面の診断修復、
README / 用途別ガイド / PrebakedPack の確認に進めます。

すぐ作る場合は、最高品質確認用として直接このメニューから作成できます。

`Tools > Siliq Water > かんたん作成 > 最高品質 Hero 水面を作成`

これだけで、分割済み水面メッシュ、Hero 専用 normal / height / caustics を使う透明水マテリアル、`WaterSurfaceAnimator`、
...
右クリック適用時は、水の種類ごとに微速の初期値が入ります。Hero / Crystal Lagoon / 透明プールは特に低速です。速く見える場合はまず **速さ** を 0.01 以下、**高さの速度** を 0.0002 以下、**水底の光の速度** を 0.00004 以下まで下げてください。
...
薄すぎる場合は水面マップスタジオの **不透明度** を 0.5 以上に上げるか、用途別の `クリスタルラグーン Hero` / `クリスタルラグーン` / `美しい海` / `透明プール` / `室内ブループール` / `フラッグシップ透明水` を使ってください。
