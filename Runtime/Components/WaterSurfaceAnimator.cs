...
namespace Siliq.Water
{
    ...
    public class WaterSurfaceAnimator : MonoBehaviour
    {
        ...
        const float SurfaceSpeedToUvPerSecond = 0.08f;
        ...
        [Tooltip("流れる速さ。0で静止、0.003が静かな水、0.006が標準、0.015以上は速め。内部で水面向けに強く減速されます。")]
        [Range(0f, MaxSurfaceSpeed)] public float speed = 0.006f;
        ...
        [Tooltip("高さ変位の動く速さ。")]
        [Range(0f, 1f)] public float displacementSpeed = 0.005f;
        ...
        [Tooltip("正面から見た時の透明な抜け感。高いほど水色の濁りを抑え、透過光と水底が見えやすくなります。Siliq 水シェーダーで有効。")]
        [Range(0f, 1f)] public float clarity = 0.55f;
        ...
        [Range(0f, 0.25f)] public float causticsSpeed = 0.001f;
        ...
        void CachePropertyIds()
        {
            ...
            clarityPropertyId = Shader.PropertyToID("_Clarity");
        ...
        void ApplyLookControls()
        {
            ...
            if (targetMaterial.HasProperty(clarityPropertyId))
            {
                propertyBlock.SetFloat(clarityPropertyId, Mathf.Clamp01(clarity));
            }
